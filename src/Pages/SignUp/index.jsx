...
import OrLine from './../../Components/OrLine/index';
...
export default class SignUp extends Component {
    ...
    schema = yup.object().shape({
        ...
        password: yup.string().min(8,"password must be more than 8")
            ...
            <Container>
                ...
                <div className="content-right-section">
                    <form onSubmit={this.handleSubmit}>
                        ...
                        {this.state.isValid? <Error className="success">Done Successfully</Error> : null}
                            <div className='title'>
                            ...
                            <Input
                                ...
                                {this.state.errors.name &&this.state.isValid===false ? <Error className="errors">{this.state.errors.name}</Error> : null}
                                ...
                                {this.state.errors.email &&this.state.isValid===false ? <Error className="errors">{this.state.errors.email}</Error> : null}
                                ...
                                <Input 
                                    ...
                                    name="password" 
                                    handleChange={(e)=>{this.handleChange(e); this.calculatePasswordStrength(this.state.password)}}>
                                    Create password*
                                </Input>
                                {this.state.errors.password &&!(this.state.isValid)   ? <Error className="errors">{this.state.errors.password}</Error> : null}
                                <div className="pass-strength">
                                    {this.state.password!==""?<Progress score={this.state.score} />:null}
                                </div>
                                ...
                                {this.state.errors.repeatPassword&&this.state.isValid===false ? <Error className="errors">{this.state.errors.repeatPassword}</Error> : null}


                                <div className="check-box">
                                    <label>
                                    <input type="checkbox" name="selected" value={this.state.selected} onChange={this.toggleCheckbox}/>
                                    I agree to terms & conditions</label>
                                {this.state.errors.selected&&this.state.isValid===false  ? <Error className="errors">{this.state.errors.selected}</Error> : null}
